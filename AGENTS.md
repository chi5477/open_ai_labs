...
- 實驗資料夾命名以遞增數字前綴（例如：`01-token-count`）
...
- 固定段落（順序如下）：
  ...
  4. `## 實驗總覽`：逐項列出子目錄與對應說明（例如：`01-token-count`）
  5. `## 共通前置條件`：適用於多數實驗的共通需求
  6. `## 注意事項`：安全或操作注意事項
- 書寫規則：
  - 以 Markdown 標題層級清晰呈現（不跳級）
  - 段落精簡、避免冗長敘述
  - 使用相對路徑連結子目錄（例如：`./01-token-count/README.md`）
...
- 固定段落（順序如下；可依實驗數量重複「實驗」區塊）：
  1. `# <實驗名稱>`：單一 H1 標題
  2. `## 範圍說明`：本文件涵蓋哪些實驗/版本
  3. `## 語言/版本`：必填，記錄此實驗使用語言與版本（例如：Python 3.12）
  4. `## 實驗 X.Y：<標題>`：每個實驗一個區塊（可重複多個）
     - X 為大實驗編號（對應子目錄，例如 `01-token-count` → X=1）
...
- 若要執行範例，請依各子目錄 `README.md` 的步驟進行（例如：`01-token-count/README.md`）。
